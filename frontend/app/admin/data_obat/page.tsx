...
import {
  Pill, PackageX, AlertTriangle, PackageCheck, Boxes,
...
function ObatModal({ onClose, onSave, existing, loading, error }: {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
      <div style={{ background: "#fff", borderRadius: "12px", width: "100%", maxWidth: "480px", boxShadow: "0 8px 32px rgba(0,0,0,.18)", fontFamily: "inherit", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderBottom: "1px solid #f0f0f0" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: "#e8f5e9", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Pill size={16} color={G} />
            </div>
            <span style={{ fontWeight: 700, fontSize: "15px", color: "#1a1a1a" }}>
              {existing ? "Edit Obat" : "Tambah Obat"}
            </span>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", fontSize: "18px", cursor: "pointer", color: "#888", display: "flex", alignItems: "center", justifyContent: "center", width: 28, height: 28, borderRadius: "50%" }}>✕</button>
        </div>

        {error && (
          <div style={{ background: "#ffebee", borderBottom: "1px solid #ffcdd2", padding: "8px 20px", fontSize: "12px", color: "#c62828", display: "flex", alignItems: "center", gap: 6 }}>
            <AlertTriangle size={13} color="#c62828" /> {error}
          </div>
        )}
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", padding: "14px 20px", borderTop: "1px solid #f0f0f0" }}>
          ...
          <button disabled={loading} onClick={() => { if (f.nama_obat) onSave(f); }}
            style={{ padding: "7px 18px", borderRadius: "8px", border: "none", background: loading ? "#81c784" : G, color: "#fff", fontSize: "13px", fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 6 }}>
            <Pill size={13} /> {loading ? "Menyimpan..." : "Simpan"}
          </button>
        </div>
      </div>
    </div>
...
export default function StokObatPage() {
  ...
  async function fetchObat() {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/obat?${params.toString()}`, { headers: { "Authorization": `Bearer ${token}` } });
  ...
  async function handleTambah(f: FormObat) {
    ...
    try {
      const res = await fetch(`${API_URL}/obat`, { method: "POST", headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify(f) });
      const data = await res.json();
      if (!res.ok) { setModalError(data.message || "Gagal menambah obat"); return; }
      await fetchObat(); setShowModal(false);
  ...
  async function handleEdit(f: FormObat) {
    ...
    setSubmitLoad(true); setModalError("");
    try {
      const res = await fetch(`${API_URL}/obat/${editTarget.id_obat}`, { method: "PUT", headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify(f) });
      const data = await res.json();
  ...
  async function handleHapus(id: number) {
    ...
    try {
      const res = await fetch(`${API_URL}/obat/${id}`, { method: "DELETE", headers: { "Authorization": `Bearer ${token}` } });
      if (!res.ok) throw new Error();
  ...
  const filtered = obatList.filter(o => filterStatus === "Semua" || stokStatus(o) === filterStatus);
  const kategoriList = Array.from(new Set(obatList.map(o => o.kategori ?? ""))).sort();
  const totalPages   = Math.max(1, Math.ceil(filtered.length / ITEMS_PER_PAGE));
  const paginated    = filtered.slice((page - 1) * ITEMS_PER_PAGE, page * ITEMS_PER_PAGE);

  const stats = [
    { label: "Total Jenis Obat", value: obatList.length,                                                 key: "Semua",  bg: "#fff",    border: "#e8e8e8", valColor: "#1a1a1a", labelColor: "#888",    icon: <Boxes        size={20} color="#555"    /> },
    { label: "Obat Habis",       value: obatList.filter(o => o.stok === 0).length,                       key: "Habis",  bg: "#ffebee", border: "#ffcdd2", valColor: "#b71c1c", labelColor: "#c62828", icon: <PackageX     size={20} color="#c62828" /> },
    { label: "Stok Kritis",      value: obatList.filter(o => o.stok > 0 && o.stok < o.min_stok).length, key: "Kritis", bg: "#fff8e1", border: "#ffe082", valColor: "#bf360c", labelColor: "#e65100", icon: <AlertTriangle size={20} color="#e65100" /> },
    { label: "Stok Aman",        value: obatList.filter(o => o.stok >= o.min_stok).length,               key: "Aman",   bg: "#e8f5e9", border: "#c8e6c9", valColor: "#1b5e20", labelColor: "#2e7d32", icon: <PackageCheck size={20} color={G}       /> },
  ];
    ...
    <div style={{ display: "flex", height: "100vh", backgroundColor: "#f5f7f5", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <main style={{ flex: 1, overflowY: "auto", background: "#f5f7f5", padding: 24 }}>
          ...
          {error && (
            <div style={{ background: "#ffebee", border: "1px solid #ffcdd2", borderRadius: 8, padding: "10px 14px", marginBottom: 16, fontSize: 13, color: "#c62828", display: "flex", alignItems: "center", gap: 8 }}>
              <AlertTriangle size={14} color="#c62828" /> {error}
            </div>
          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 20 }}>
            {stats.map(stat => (
              <div key={stat.key} onClick={() => { setFilterStatus(stat.key); setPage(1); }}
                style={{ background: stat.bg, border: `1.5px solid ${filterStatus === stat.key ? G : stat.border}`, borderRadius: 10, padding: "16px 20px", cursor: "pointer", transition: "border 0.15s", display: "flex", alignItems: "center", gap: 14 }}
              >
                <div style={{ width: 42, height: 42, borderRadius: 10, background: "#fff", border: `1px solid ${stat.border}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  {stat.icon}
                </div>
                <div>
                  <p style={{ fontSize: 12, color: stat.labelColor, margin: 0, fontWeight: 500 }}>{stat.label}</p>
                  <p style={{ fontSize: 26, fontWeight: 700, color: stat.valColor, margin: "2px 0 0" }}>{stat.value}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr auto", gap: 14, alignItems: "flex-end" }}>
              <div>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input style={{ ...inputStyle, paddingLeft: 30 }} placeholder="Cari nama obat..." value={search} onChange={e => setSearch(e.target.value)} />
                </div>
              </div>
              <div>
                <label style={filterLabel}>Kategori</label>
                <div style={{ position: "relative" }}>
                  <FlaskConical size={13} color="#aaa" style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)" }} />
                  <select style={{ ...inputStyle, paddingLeft: 30 }} value={filterKat} onChange={e => setFilterKat(e.target.value)}>
              ...
              <div>
                ...
                <div style={{ position: "relative" }}>
                  <SlidersHorizontal size={13} color="#aaa" style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)" }} />
                  <select style={{ ...inputStyle, paddingLeft: 30 }} value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
              ...
              <div>
                <button onClick={openAdd} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 8, border: "none", background: G, color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", height: 36, whiteSpace: "nowrap" }}>
          ...
          <div style={{ ...card, padding: 0, overflow: "hidden" }}>
            {loading ? (
              <div style={{ padding: 40, textAlign: "center", color: "#888", display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                <Pill size={32} color="#c8e6c9" />
                <span style={{ fontSize: 13 }}>Memuat data obat...</span>
              </div>
            ) : (
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    {paginated.length === 0 ? (
                      <tr>
                        <td colSpan={8} style={{ padding: "3rem", textAlign: "center", color: "#999", fontSize: 13 }}>
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                            <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#f0faf2", border: "2px solid #c8e6c9", display: "flex", alignItems: "center", justifyContent: "center" }}>
                              <Pill size={26} color="#a5d6a7" />
                            </div>
                      ...
                      return (
                        ...
                        >
                          <td style={td}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              <div style={{ width: 30, height: 30, borderRadius: 7, background: "#f0faf2", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                                <Pill size={14} color={G} />
                              </div>
                              <div>
                          ...
                          <td style={td}>
                            <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, background: "#f5f5f5", padding: "2px 8px", borderRadius: 20, color: "#555" }}>
                              <FlaskConical size={11} color="#888" /> {item.kategori ?? "-"}
                            </span>
                          </td>
                          ...
                          <td style={td}>
                            <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, background: stC.bg, color: stC.color, whiteSpace: "nowrap" }}>
                              {st === "Aman"   && <PackageCheck  size={11} />}
                              {st === "Kritis" && <AlertTriangle size={11} />}
                              {st === "Habis"  && <PackageX      size={11} />}
                              {st}
                            </span>
                          </td>
                          <td style={td}>
                            <div style={{ display: "flex", gap: 6 }}>
                              <button onClick={() => openEdit(item)} style={{ padding: "4px 10px", borderRadius: 7, border: `1px solid #c8e6c9`, background: "#fff", fontSize: 12, fontWeight: 600, color: G, cursor: "pointer", fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 4 }}>
                                <Pencil size={11} /> Edit
                              </button>
                              <button onClick={() => handleHapus(item.id_obat)} style={{ padding: "4px 10px", borderRadius: 7, border: "1px solid #ffcdd2", background: "#fff", fontSize: 12, fontWeight: 600, color: "#c62828", cursor: "pointer", fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 4 }}>
