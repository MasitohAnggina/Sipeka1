            ...
            >
              <div
                style={{
                  ...
                  fontSize: 14,
                  flexShrink: 0,
                  background: done ? "#4caf50" : active ? G : "#c8e6c9",
                  color: "#fff",
                }}
              >
...
function Step1({
    ...
    return (
      <div style={{ textAlign: "center", padding: "40px 0", color: "#888" }}>
        ...
        <div style={{ fontSize: 13 }}>
          Tambahkan hewan di menu <strong>Data Hewan</strong> terlebih dahulu.
        </div>
      </div>
    );
  }
  return (
    <div>
      <h2 style={{ color: G, fontSize: 17, fontWeight: 700, marginBottom: 4 }}>
      ...
      {sel.length > 0 && (
        ...
        >
          ...
          {sel.length} hewan dipilih:{" "}
          {sel.map((id) => pets.find((p) => p.id === id)?.name).join(", ")}
        </div>
      )}
    </div>
  );
...
function Step2({
  ...
  onToggleSvc(petId: string, layananId: string): void;
  ...
  return (
    <div>
      ...
      >
        ...
        <span>
          Pilih layanan untuk setiap hewan. Layanan di bawah diambil langsung
          dari klinik.
        </span>
      </div>
      ...
      {services.length === 0 ? (
        ...
        >
          {services.map((layanan, idx) => {
            const checked = selected.includes(String(layanan.id_layanan));
            const katColor = getKategoriColor(layanan.kategori);
            return (
              <div
                key={layanan.id_layanan}
                onClick={() => onToggleSvc(tab, String(layanan.id_layanan))}
                style={{
                  display: "flex",
                  alignItems: "center",
                  gap: 14,
      ...
      <div>
        ...
        >
          Catatan untuk {pet.name}{" "}
          <span style={{ fontWeight: 400, color: "#999" }}>(opsional)</span>
        </label>
        <textarea
          value={notes[tab] || ""}
...
function Step3FotoKondisi({
  ...
  setCondPhotos: React.Dispatch<
    React.SetStateAction<Record<string, ConditionPhoto[]>>
  >;
}) {
  const [tab, setTab] = useState(sel[0]);
  ...
  const handleFile = async (file: File | null, label: "before" | "after") => {
    ...
    setCondPhotos((prev) => ({
      ...prev,
      [tab]: [
        ...(prev[tab] ?? []).filter((p) => p.label !== label),
        { id: Date.now() + "", dataUrl, label },
      ],
    }));
  };
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        <h2
          style={{ color: G, fontSize: 17, fontWeight: 700, marginBottom: 4 }}
        >
          Foto Kondisi Hewan
        </h2>
        <div
        ...
        >
          <Info size={15} style={{ flexShrink: 0, marginTop: 1 }} />
          <span>
            <strong>Opsional.</strong> Upload foto kondisi hewan — sebelum sakit
            (normal/sehat) dan saat sakit (kondisi terkini).
          </span>
        </div>
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <PhotoSlot
          ...
          onFileChange={(f) => handleFile(f, "before")}
        />
      </div>
      ...
      <div>
        ...
        >
          ...
          />
          <span style={{ fontWeight: 700, fontSize: 14, color: "#c62828" }}>
            Saat Sakit
          </span>
          <span style={{ fontSize: 12, color: "#888" }}>
            — kondisi terkini yang perlu ditangani
          </span>
        </div>
        <PhotoSlot
          photo={afterPhoto}
          ...
          onFileChange={(f) => handleFile(f, "after")}
        />
      </div>
    </div>
  );
...
function Step4Jadwal({
  ...
  return (
    <div>
      <h2 style={{ color: G, fontSize: 17, fontWeight: 700, marginBottom: 16 }}>
              ...
              return (
                ...
                >
                  ...
                  {j.jam_mulai && j.jam_selesai && (
                    ...
                    >
                      <Clock size={11} color="#888" /> {j.jam_mulai} –{" "}
                      {j.jam_selesai}
                    </div>
                  )}
                </div>
          ...
          return (
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                {layananDipilih.length === 0 ? (
                  <span style={{ color: "#aaa" }}>–</span>
                ) : (
                  layananDipilih.map((l) => {
                    const katColor = getKategoriColor(l.kategori);
                    return (
                      <div
                        key={l.id}
                        style={{
      ...
      >
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 2 }}>
            Persetujuan Umum — Berlaku untuk Semua Hewan
          </div>
  ...
  return (
    <div>
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontWeight: 700, fontSize: 15 }}>
              {selectedJadwal
                ? `${selectedJadwal.hari}, ${fmtDate(selectedJadwal.tanggal)}`
                : "–"}
            </div>
      ...
      <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 10 }}>
        Detail per Hewan ({sel.length} hewan)
      </div>
      {sel.map((id) => {
        const p = pets.find((x) => x.id === id)!;
        const layananDipilih = services.filter((s) =>
        ...
        return (
          ...
          >
            ...
            {cphs.length > 0 && (
              <div
                style={{ padding: "9px 15px", borderTop: "1px solid #f0f0f0" }}
              >
                <div
                  style={{
                    fontSize: 12,
...
function Success({
  ...
  return (
    <div style={{ textAlign: "center", padding: "34px 0" }}>
      ...
      <h2 style={{ color: G, fontSize: 20, fontWeight: 700, marginBottom: 4 }}>
        Booking Berhasil!
      </h2>
      <p style={{ fontSize: 15, marginBottom: 12 }}>
        No. Booking:{" "}
        <strong style={{ color: G }}>#{firstBooking?.no_booking}</strong>
        {bookings.length > 1 && (
          <span style={{ fontSize: 12, color: "#888", marginLeft: 6 }}>
            +{bookings.length - 1} lainnya
          </span>
        )}
      </p>
      <div
      ...
      >
        <div style={{ textAlign: "left" }}>
          ...
          <div
            style={{ fontSize: 42, fontWeight: 800, color: G, lineHeight: 1 }}
          >
            {String(firstBooking?.no_antrian ?? 0).padStart(3, "0")}
          </div>
          <div style={{ fontSize: 11, color: "#888" }}>
            Tunjukkan ke petugas klinik
          </div>
        </div>
      </div>
...
export default function BookingPage() {
  ...
  const toggleSvc = (petId: string, layananId: string) => {
    setSvc((prev) => {
      ...
      const updated = current.includes(layananId)
        ? current.filter((x) => x !== layananId)
        : [...current, layananId];
    ...
    <div style={{ display: "flex", height: "100vh", overflow: "hidden" }}>
      ...
      >
        <Header
          ...
          subtitle="Booking Layanan Untuk Hewan Kesayangan Anda"
        />
        ...
        <div style={{ padding: "0 28px 28px" }}>
          ...
          >
            ...
            {done ? (
              ...
              <>
                {step === 1 && (
                  <Step1 pets={pets} sel={sel} toggle={togglePet} />
                )}
                {step === 2 && (
                  <Step2
                    pets={pets}
                ...
                {step === 3 && (
                  <Step3FotoKondisi
                    pets={pets}
                    sel={sel}
                    condPhotos={condPhotos}
                    setCondPhotos={setCondPhotos}
                  />
                )}
                {step === 4 && (
                  <Step4Jadwal
