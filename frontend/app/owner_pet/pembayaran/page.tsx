...
import {
  ...
  RefreshCw, Info, Search, RotateCcw,
} from "lucide-react";
...
function getDateRange(filter: string, customDate: string): { from: Date | null; to: Date | null } {
  const now   = new Date();
...
function SkeletonPembayaran() {
  return (
    <>
      <style>{`
        @keyframes sk-pay { 0% { background-position: -600px 0; } 100% { background-position: 600px 0; } }
        .sk-pay { background: linear-gradient(90deg, #e8e8e8 25%, #f2f2f2 50%, #e8e8e8 75%); background-size: 1200px 100%; animation: sk-pay 1.5s ease-in-out infinite; border-radius: 6px; }
      `}</style>
      ...
      <div style={{ background: "#fff", borderRadius: 12, padding: "12px 18px", marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.07)", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        {[50, 160, 140, 140, 80].map((w, i) => (
          <div key={i} className="sk-pay" style={{ height: 34, width: w, borderRadius: 8 }} />
        ))}
        <div className="sk-pay" style={{ height: 32, width: 80, borderRadius: 8, marginLeft: "auto" }} />
      </div>
      {[0, 1].map(i => (
...
> = {
  menunggu_pembayaran: { label: "Belum Dibayar",         color: "#b45309", bg: "#fef3c7", icon: <Clock          style={{ width: 11, height: 11 }} /> },
  pending_cash:        { label: "Menunggu Kasir",         color: "#1d4ed8", bg: "#eff6ff", icon: <Banknote       style={{ width: 11, height: 11 }} /> },
  pending_midtrans:    { label: "Menunggu Pembayaran",    color: "#7c3aed", bg: "#f5f3ff", icon: <Smartphone     style={{ width: 11, height: 11 }} /> },
  lunas:               { label: "Lunas",                  color: G,         bg: "#f0faf2", icon: <CheckCircle2   style={{ width: 11, height: 11 }} /> },
  gagal:               { label: "Gagal / Expired",        color: "#dc2626", bg: "#fef2f2", icon: <AlertCircle    style={{ width: 11, height: 11 }} /> },
};

function StatusBadge({ status }: { status: Pembayaran["status"] }) {
  const cfg = STATUS_CFG[status];
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 600, color: cfg.color, background: cfg.bg, border: `1px solid ${cfg.color}33`, borderRadius: 20, padding: "3px 9px" }}>
      {cfg.icon} {cfg.label}
    </span>
  );
}
...
function ModalBayar({ invoice, onClose, onBayar }: { invoice: Invoice; onClose(): void; onBayar(metode: "cash" | "midtrans"): Promise<void> }) {
  const [loading, setLoading] = useState(false);
  const [chosen,  setChosen]  = useState<"cash" | "midtrans" | null>(null);
  const [error,   setError]   = useState<string | null>(null);

  async function handlePilih(metode: "cash" | "midtrans") {
    setChosen(metode); setLoading(true); setError(null);
    try { await onBayar(metode); }
    catch (e: unknown) { setError(e instanceof Error ? e.message : "Terjadi kesalahan"); setChosen(null); }
    finally { setLoading(false); }
  }

  const isBisa = invoice.pembayaran.status === "menunggu_pembayaran" || invoice.pembayaran.status === "gagal" || invoice.pembayaran.status === "pending_midtrans";

  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 440, boxShadow: "0 24px 64px rgba(0,0,0,0.18)", fontFamily: "'Inter', sans-serif", overflow: "hidden" }}>
        <div style={{ padding: "18px 20px", borderBottom: "1px solid #f0f0f0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: "#f0faf2", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <button onClick={onClose} style={{ width: 28, height: 28, borderRadius: "50%", border: "none", background: "#f5f5f5", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#888" }}>
            <X style={{ width: 14, height: 14 }} />
          </button>
        </div>
        <div style={{ padding: "16px 20px" }}>
          <div style={{ background: "linear-gradient(135deg, #f0faf2, #e8f5e9)", border: "1px solid #c8e6c9", borderRadius: 10, padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <div>
          ...
          {error && (
            <div style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "10px 12px", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 8, marginBottom: 14 }}>
              <AlertCircle style={{ width: 14, height: 14, color: "#dc2626", flexShrink: 0, marginTop: 1 }} />
              <span style={{ fontSize: 12, color: "#dc2626" }}>{error}</span>
            </div>
          )}
          {!isBisa ? (
            <div style={{ padding: "14px 16px", background: "#f0faf2", border: "1px solid #a5d6a7", borderRadius: 10, display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: G }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {(["cash", "midtrans"] as const).map(metode => {
                const isCash = metode === "cash";
                const accent = isCash ? G : "#7c3aed";
                const accentBg = isCash ? "#f0faf2" : "#f5f3ff";
                const accentBorder = isCash ? "#c8e6c9" : "#ddd6fe";
                return (
                  <button key={metode} onClick={() => handlePilih(metode)} disabled={loading}
                    style={{ width: "100%", padding: "14px 16px", borderRadius: 12, border: `1.5px solid ${chosen === metode ? accent : "#e0e0e0"}`, background: chosen === metode ? accentBg : "#fff", cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 14, textAlign: "left", fontFamily: "inherit", transition: "all 0.15s", opacity: loading && chosen !== metode ? 0.5 : 1 }}
                    onMouseEnter={e => { if (!loading) e.currentTarget.style.borderColor = accent; }}
                    onMouseLeave={e => { if (chosen !== metode) e.currentTarget.style.borderColor = "#e0e0e0"; }}
                  >
                    <div style={{ width: 44, height: 44, borderRadius: 10, background: accentBg, border: `1.5px solid ${accentBorder}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      {loading && chosen === metode
                        ? <Loader2 style={{ width: 20, height: 20, color: accent, animation: "spin 1s linear infinite" }} />
                        : isCash ? <Banknote style={{ width: 20, height: 20, color: G }} /> : <Smartphone style={{ width: 20, height: 20, color: "#7c3aed" }} />
                      }
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: "#111", marginBottom: 2 }}>
                        {isCash ? "Bayar Tunai (Cash)" : <>Bayar Digital <span style={{ marginLeft: 6, fontSize: 10, color: "#7c3aed", background: "#ede9fe", borderRadius: 4, padding: "1px 6px" }}>QRIS · GoPay · Transfer</span></>}
                      </div>
                      <div style={{ fontSize: 12, color: "#888", lineHeight: 1.4 }}>
                        {isCash ? "Lakukan pembayaran langsung di kasir klinik" : "QRIS, GoPay, ShopeePay, Transfer Bank, dan lainnya"}
                      </div>
                    </div>
...
function ModalCashInfo({ invoice, onClose }: { invoice: Invoice; onClose(): void }) {
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 420, boxShadow: "0 24px 64px rgba(0,0,0,0.18)", fontFamily: "'Inter', sans-serif", overflow: "hidden" }}>
        <div style={{ background: `linear-gradient(135deg, ${G}, #1b5e20)`, padding: "20px 20px 16px", display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.75)", marginTop: 2 }}>Silakan menuju kasir klinik untuk menyelesaikan pembayaran tunai.</div>
          </div>
        </div>
        <div style={{ padding: "16px 20px" }}>
          <div style={{ background: "#f9f9f9", border: "1px solid #f0f0f0", borderRadius: 10, padding: "12px 14px", marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#333", marginBottom: 4 }}>
              <span>No. Invoice</span><span style={{ fontWeight: 600 }}>#{invoice.id_resep}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#333" }}>
              <span>Total Tagihan</span><span style={{ fontWeight: 700, color: G }}>{fmt(invoice.grand_total)}</span>
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
            {[
              ...
              <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 7, background: "#f0faf2", display: "flex", alignItems: "center", justifyContent: "center", color: G, flexShrink: 0 }}>{item.icon}</div>
                <span style={{ fontSize: 13, color: "#444", lineHeight: 1.5, paddingTop: 4 }}>{item.text}</span>
              </div>
            ))}
          </div>
          <button onClick={onClose} style={{ width: "100%", padding: "11px", borderRadius: 10, border: "none", background: G, color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
            Mengerti
          </button>
        </div>
      </div>
...
function InvoiceCard({ invoice, onBayar }: { invoice: Invoice; onBayar(inv: Invoice): void }) {
  ...
  return (
    <div style={{ border: `1px solid ${st === "lunas" ? "#c8e6c9" : st === "menunggu_pembayaran" || st === "gagal" ? "#fed7aa" : "#e0e0e0"}`, borderRadius: 12, background: "#fff", overflow: "hidden", marginBottom: 10, boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
      <div style={{ padding: "13px 16px", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <div style={{ width: 34, height: 34, borderRadius: 8, background: "#f0faf2", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 140 }}>
          ...
          <div style={{ fontSize: 11, color: "#aaa", marginTop: 2, display: "flex", alignItems: "center", gap: 4 }}>
            <Calendar style={{ width: 10, height: 10 }} />{fmtTanggal(invoice.tanggal)}
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 15, fontWeight: 700, color: G }}>{fmt(invoice.grand_total)}</span>
          {(st === "menunggu_pembayaran" || st === "gagal") && (
            <button onClick={() => onBayar(invoice)} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "7px 14px", borderRadius: 8, border: "none", background: G, color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}
          ...
          {st === "pending_midtrans" && (
            <button onClick={() => onBayar(invoice)} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "7px 14px", borderRadius: 8, border: "1.5px solid #7c3aed", background: "#f5f3ff", color: "#7c3aed", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
              <Smartphone style={{ width: 12, height: 12 }} /> Lanjutkan
            </button>
          )}
          {st === "pending_cash" && (
            <button onClick={() => onBayar(invoice)} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "7px 14px", borderRadius: 8, border: "1.5px solid #1d4ed8", background: "#eff6ff", color: "#1d4ed8", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
              <Info style={{ width: 12, height: 12 }} /> Info
            </button>
          )}
          <button onClick={() => setExpanded(p => !p)} style={{ width: 28, height: 28, borderRadius: 7, border: "1px solid #e0e0e0", background: "#fafafa", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#888" }}>
        ...
        <div style={{ borderTop: "1px solid #f5f5f5", padding: "12px 16px" }}>
          ...
          {invoice.catatan && (
            <div style={{ marginTop: 8, padding: "8px 12px", background: "#fffde7", borderRadius: 8, fontSize: 12, color: "#555", lineHeight: 1.6 }}>
              <span style={{ fontWeight: 600, color: "#888" }}>Catatan dokter: </span>{invoice.catatan}
            </div>
          )}
        </div>
      )}
...
function HewanSection({ data, onBayar }: { data: HewanInvoice; onBayar(inv: Invoice): void }) {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e8e8e8", marginBottom: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", overflow: "hidden" }}>
      <button onClick={() => setOpen(p => !p)} style={{ width: "100%", padding: "14px 18px", display: "flex", alignItems: "center", gap: 12, background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", textAlign: "left", borderBottom: open ? "1px solid #f0f0f0" : "none" }}>
        {data.foto ? (
...
export default function PembayaranPage() {
  ...
  const fetchInvoice = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch(`${API_BASE}/owner/invoice`, { headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" } });
  ...
  async function handleBayar(metode: "cash" | "midtrans") {
    ...
    if (metode === "cash") {
      await fetchInvoice();
      setModalInv(null);
      setCashInv({ ...modalInv, pembayaran: { ...modalInv.pembayaran, status: "pending_cash", metode: "cash" } });
      return;
    }
  ...
  // ── Summary stats ──
  const allInvoices   = data.flatMap(h => h.invoices);
  const totalBelumBayar       = allInvoices.filter(inv => inv.pembayaran.status === "menunggu_pembayaran" || inv.pembayaran.status === "gagal").length;
  const totalLunas             = allInvoices.filter(inv => inv.pembayaran.status === "lunas").length;
  const totalNominalBelumBayar = allInvoices.filter(inv => inv.pembayaran.status === "menunggu_pembayaran" || inv.pembayaran.status === "gagal").reduce((s, inv) => s + inv.grand_total, 0);

  // ── Filter logic ──
  const jenisHewanList = ["Semua Hewan", ...Array.from(new Set(data.map(h => h.nama_hewan)))];

  const statusOptions = [
    { label: "Semua Status",   value: "Semua" },
    { label: "Belum Dibayar",  value: "menunggu_pembayaran" },
    { label: "Menunggu Kasir", value: "pending_cash" },
    { label: "Lunas",          value: "lunas" },
  ];

  const filteredData: HewanInvoice[] = data
    .filter(h => filterHewan === "Semua Hewan" || h.nama_hewan === filterHewan)
    .map(h => ({
      ...h,
      invoices: h.invoices.filter(inv => {
        // Filter status
        const matchStatus = filterStatus === "Semua" || inv.pembayaran.status === filterStatus;

        // Filter tanggal
        const { from, to } = getDateRange(filterTanggal, customDate);
        const matchTanggal = !from || !to
          ? true
          : (() => { const tgl = new Date(inv.tanggal); return tgl >= from && tgl <= to; })();

        return matchStatus && matchTanggal;
      }),
    }))
    .filter(h => h.invoices.length > 0);

  const totalFiltered = filteredData.reduce((s, h) => s + h.invoices.length, 0);
  const hasFilter = filterStatus !== "Semua" || filterTanggal !== "Semua" || filterHewan !== "Semua Hewan" || customDate !== "";
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 22 }}>
                {[
                  ...
                  { icon: <Receipt style={{ width: 18, height: 18, color: "#1565c0" }} />,     bg: "#eff6ff", border: "#bfdbfe", label: "Total Invoice",  value: allInvoices.length,                                                       sub: `${data.length} hewan`,                                             valueColor: "#1565c0" },
                ].map((card, i) => (
              ...
              <div style={{ background: "#fff", borderRadius: 12, padding: "12px 18px", marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.07)", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                <div style={filterWrapStyle}>
                  ...
                  <select value={filterHewan} onChange={e => setFilterHewan(e.target.value)} style={selectStyle}>
                ...
                <div style={filterWrapStyle}>
                  ...
                  <select value={filterTanggal} onChange={e => { setFilterTanggal(e.target.value); if (e.target.value !== "Custom") setCustomDate(""); }} style={selectStyle}>
                    {["Semua", "Hari Ini", "1 Minggu", "1 Bulan", "Custom"].map(o => (
                ...
                {filterTanggal === "Custom" && (
                  <div style={filterWrapStyle}>
                    <Calendar size={13} color="#666" />
                    <input type="date" value={customDate} onChange={e => setCustomDate(e.target.value)} style={selectStyle} />
                  </div>
                )}

                {/* Reset */}
                <button
                  onClick={() => { setFilterStatus("Semua"); setFilterTanggal("Semua"); setFilterHewan("Semua Hewan"); setCustomDate(""); }}
                  style={{ padding: "7px 14px", borderRadius: 8, border: "none", background: "#fce4ec", color: "#c62828", fontSize: 13, fontWeight: 700, cursor: "pointer", marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 6 }}
                >
                  <RotateCcw size={13} color="#c62828" /> Reset
                </button>
              </div>

              {/* Hasil filter info + Refresh */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                {hasFilter ? (
                  <span style={{ fontSize: 13, color: "#888" }}>
                    Menampilkan <strong style={{ color: "#333" }}>{totalFiltered}</strong> invoice dari filter yang dipilih
                  </span>
                ) : <span />}
                <button onClick={fetchInvoice} disabled={loading} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 13px", borderRadius: 8, border: "1px solid #e0e0e0", background: "#fff", color: "#555", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>
              ...
              {!error && filteredData.length === 0 && (
                <div style={{ textAlign: "center", padding: "4rem 2rem", background: "#fff", borderRadius: 14, border: "1px solid #e8e8e8" }}>
                  ...
                  <div style={{ fontSize: 15, fontWeight: 600, color: "#1a1a1a", marginBottom: 6 }}>
                    {hasFilter ? "Tidak ada invoice untuk filter ini" : "Belum ada invoice"}
                  </div>
                  <div style={{ fontSize: 13, color: "#888" }}>
                    {hasFilter ? "Coba ubah atau reset filter di atas." : "Invoice akan muncul setelah dokter membuat resep untuk hewan kamu."}
                  </div>
                </div>
              )}
