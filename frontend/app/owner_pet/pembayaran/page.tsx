          ...
          {!isBisa ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <button
                ...
                onMouseEnter={e => {
                  if (!loading) e.currentTarget.style.borderColor = G;
                }}
                onMouseLeave={e => {
                  if (chosen !== "cash") e.currentTarget.style.borderColor = "#e0e0e0";
                }}
              >
                <div style={{
              ...
              <button
                ...
                onMouseEnter={e => {
                  if (!loading) e.currentTarget.style.borderColor = "#7c3aed";
                }}
                onMouseLeave={e => {
                  if (chosen !== "midtrans") e.currentTarget.style.borderColor = "#e0e0e0";
                }}
              >
                <div style={{
                  width: 44, height: 44, borderRadius: 10,
...
function InvoiceCard({
  ...
  const totalLayanan = invoice.details
    .filter(d => d.tipe === "layanan")
    .reduce((s, d) => s + d.subtotal, 0);
  const totalObat = invoice.details
    .filter(d => d.tipe === "obat")
    .reduce((s, d) => s + d.subtotal, 0);
      ...
      {expanded && (
        <div style={{ borderTop: "1px solid #f5f5f5", padding: "12px 16px" }}>
          ...
          <div style={{
            ...
            padding: "10px 12px", background: "#f0faf2",
...
function HewanSection({
  ...
  const totalTagihan = data.invoices
    .filter(inv => inv.pembayaran.status !== "lunas")
    .reduce((s, inv) => s + inv.grand_total, 0);
  const adaTagihan = data.invoices.some(
    inv => inv.pembayaran.status === "menunggu_pembayaran" || inv.pembayaran.status === "gagal"
  );
  ...
  return (
    <div style={{
      ...
      >
        {data.foto ? (
          <img src={data.foto} alt={data.nama_hewan}
            style={{ width: 46, height: 46, borderRadius: "50%", objectFit: "cover", flexShrink: 0, border: "2px solid #c8e6c9" }} />
        ) : (
          <div style={{
            width: 46, height: 46, borderRadius: "50%",
            background: "#e8f5e9", border: "2px solid #c8e6c9",
            display: "flex", alignItems: "center", justifyContent: "center",
            flexShrink: 0, color: G,
          }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 11, color: "#888", background: "#f4f4f4", borderRadius: 20, padding: "2px 8px" }}>
              {data.jenis} · {data.ras}
            </span>
          </div>
        ...
        {open
          ? <ChevronUp style={{ width: 16, height: 16, color: "#bbb", flexShrink: 0 }} />
          : <ChevronDown style={{ width: 16, height: 16, color: "#bbb", flexShrink: 0 }} />
        }
      </button>
        ...
        <div style={{ padding: "12px 16px" }}>
          {data.invoices.length === 0 ? (
            ...
            data.invoices.map(inv => (
              <InvoiceCard key={inv.id_resep} invoice={inv} onBayar={onBayar} />
            ))
          )}
        </div>
      )}
...
export default function PembayaranPage() {
  ...
  const totalBelumBayar = data.reduce((s, h) =>
    s + h.invoices.filter(inv =>
      inv.pembayaran.status === "menunggu_pembayaran" ||
      inv.pembayaran.status === "gagal"
    ).length, 0);
  ...
  const totalNominalBelumBayar = data.reduce((s, h) =>
    s + h.invoices
      ...
      .reduce((ss, inv) => ss + inv.grand_total, 0), 0);
  ...
  return (
    <div style={{ display: "flex", height: "100vh", overflow: "hidden", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "auto", background: "#f5f7f5" }}>
        <Header
          title="Pembayaran"
          subtitle="Lihat tagihan dan selesaikan pembayaran klinik"
        />
        ...
        <div style={{ padding: "22px 24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 22 }}>
            {[
              {
                icon: <AlertCircle style={{ width: 18, height: 18, color: "#b45309" }} />,
              ...
              {
                icon: <CheckCircle2 style={{ width: 18, height: 18, color: G }} />,
                bg: "#f0faf2", border: "#c8e6c9",
                label: "Sudah Lunas",
                value: totalLunas,
                sub: "invoice",
                valueColor: G,
              ...
              {
                icon: <Receipt style={{ width: 18, height: 18, color: "#1565c0" }} />,
                bg: "#eff6ff", border: "#bfdbfe",
                label: "Total Invoice",
                value: data.reduce((s, h) => s + h.invoices.length, 0),
                sub: `${data.length} hewan`,
                valueColor: "#1565c0",
              },
            ].map((card, i) => (
              <div key={i} style={{
                ...
                <div>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 700, color: card.valueColor, lineHeight: 1 }}>
                    {card.value}
                  </div>
  ...
  suppressHydrationWarning
              style={{
                ...
                cursor: (!mounted || loading) ? "not-allowed" : "pointer",
                fontFamily: "inherit",
              }}
            >
          ...
          {/* Loading */}
          {loading && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "4rem", color: "#888", fontSize: 13 }}>
          ...
          {/* Hewan sections */}
          {mounted && !loading && data.map(hewan => (
            <HewanSection
              key={hewan.id_hewan}
              data={hewan}
              onBayar={inv => setModalInv(inv)}
            />
          ))}
        </div>
      </div>
      ...
      {(cashInv || (modalInv && modalInv.pembayaran.status === "pending_cash")) && (
        <ModalCashInfo
          invoice={(cashInv ?? modalInv)!}
          onClose={() => { setCashInv(null); setModalInv(null); }}
        />
      )}
    </div>
  );
