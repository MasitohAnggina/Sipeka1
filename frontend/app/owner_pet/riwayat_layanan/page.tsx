      ...
      >
        ...
        <div style={{ background: "#fff", padding: "14px 20px", borderBottom: "1px solid #ebebeb", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 46, height: 46, borderRadius: 10, background: G, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <AnimalIcon jenis={hewan?.jenis ?? ""} size={26} />
            </div>
            <div>
          ...
          <button
            onClick={onClose}
            style={{ width: 32, height: 32, borderRadius: "50%", border: "none", background: "#f5f5f5", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#888" }}
          >
            <X size={16} color="#888" />
          </button>
        </div>
                ...
                <div style={cardS}>
                  ...
                  {item.layanans.length === 0 ? (
                    ...
                    <div key={l.id_layanan} style={{
                      ...
                      <div style={{ fontSize: 13, fontWeight: 700, color: G }}>{toRp(l.harga_saat_booking)}</div>
                    </div>
                  ))}
                    ...
                    {(rm.tindakanList ?? []).length > 0 && (
                      <div style={cardS}>
                        ...
                        {(rm.tindakanList ?? []).map((t, idx) => (
                          <div key={t.id} style={{ background: "#f9f9f9", border: "1.5px solid #e0e0e0", borderRadius: 10, padding: "11px 13px", marginBottom: idx < (rm.tindakanList ?? []).length - 1 ? 8 : 0 }}>
...
function RiwayatLayananContent() {
  ...
  const handleOpenDetail = async (item: RiwayatItem) => {
    ...
    try {
      ...
      if (json.success) {
        setModalItem(prev => prev ? { ...prev, ...json.data } : null);
      }
    } catch {
      // Gagal fetch — modal tetap tampil dengan data list yang ada
    } finally {
      setLoadingDetail(false);
    }
  };
      ...
      <div style={{ background: "#fff", borderRadius: 12, padding: "12px 18px", marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.07)", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        {[
          ...
          },
          {
            label: "Jenis Hewan",
            value: filterHewan,
            options: jenisHewanList,
            onChange: (v: string) => { setFilterHewan(v); setCurrentPage(1); },
          },
        ].map(f => (
