...
export function useBookingNotif(booking: BookingAktif | null): BookingNotif[] {
  return useMemo(() => {
    if (!booking) return [];
    if (["selesai", "dibatalkan"].includes(booking.status)) return [];
    ...
    // H-1: antara 12–28 jam sebelum booking
    if (diffHrs > 12 && diffHrs <= 28) {
      notifs.push({
    ...
    // H-12: antara 0–12 jam sebelum booking
